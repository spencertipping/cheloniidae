   ...
   */
    ...
    public class TurtleCanvas extends Canvas {
      ...
      // Accessors
        ...
        public int      getDrawingIntermediateLines ()                                          {return drawingIntemediateLines;}
        ...
        // Graphical methods (available to user)
          ...
          public void enqueueGraphicsRefreshRequest (boolean drawAllLines, boolean antialiased) {
            ...
            if (imgTurtleOutput != null && imgTurtleLayer != null) {
              ...
              final TurtleCanvas _this         = this;
              thrGraphicsRequestRunner = new Thread (new Runnable () {
                public void run () {
          ...
          protected void resortLines () {
            ...
            synchronized (lines) {
              for (int i = 0; i < lines.size () && !graphicsRequestCancel; i++)
                ((Line) lines.get (i)).cachedDistance = lineDistance ((Line) lstLines.get (i));
        ...
        // Graphical methods (hidden from user)
          ...
          protected void drawLines (boolean startOver, boolean antialiased) {
            ...
            if (antialiased)
              ...
              if (lstLines.size () > drawingIntermediateLines)
                skip = lstLines.size () / drawingIntermediateLines;
              ...
              if (lines.size () > linesDrawn) {
                if (linesDrawn > 0)
                  while (linesDrawn < lines.size () && !graphicsRequestCancel)
                    renderLine ((Line) lines.get (linesDrawn++), g, imgTurtleOutput, antialiased);
                else {
                  for (int i = 0; i < lines.size () && !graphicsRequestCancel; i += skip) {
                    renderLine ((Line) lines.get (i), g, imgTurtleOutput, antialiased);
